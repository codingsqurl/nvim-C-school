...
// ===== INLINE-STYLE MARKDOWN RENDERER (for terminal) =====
...
function renderMarkdownInline(md) {
  ...
  function renderInline(text) {
    text = text.replace(/\[\[([^\]|]+)\]\]/g, function(_m, target) {
      const parts = target.split('|');
      const label = parts.length > 1 ? parts[1] : parts[0];
      const linkTarget = parts[0];
      return '<span class="wikilink" data-target="' + escapeHtml(linkTarget) + '">' + escapeHtml(label) + '</span>';
  ...
  while (i < lines.length) {
    ...
    if (trimmed.startsWith('# ')) {
      ...
      const rendered = tableRows.map(function(r) {
        const cells = r.split('|').slice(1, -1).map(function(c) { return c.trim(); });
        if (cells.every(function(c) { return /^[-:]+$/.test(c); })) return '';
        return '<span style="opacity:0.8;font-family:monospace">' + cells.map(function(c) { return escapeHtml(c); }).join(' | ') + '</span>';
